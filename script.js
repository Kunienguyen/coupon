...
function getCouponColor(coupon) {
    ...
    if (coupon.status === "used") return "gray";

    const left = coupon.expireAt - Date.now();

    if (left <= 0) return "red";

    if (left <= WARNING_TIME) return "yellow";
...
function getCouponText(coupon) {
    ...
    let total = Math.floor(left / 1000);
    ...
    return `Còn ${d} ngày ${h} giờ ${m} phút`;
}
...
function canSpin() {
    ...
    return (
        Date.now() - data.lastSpin >= FREE_SPIN_TIME
    );
}
...
function startSpin() {
    ...
    const spinAnim =
        document.getElementById("spinAnimation");

    let count = 0;

    const interval = setInterval(() => {

        spinAnim.textContent =
            COUPONS[
                Math.floor(
...
document.addEventListener(
    ...
    () => {
        ...
        document
            .getElementById("cancelCouponBtn")
            .onclick = () => {
        ...
        document
            .getElementById("cooldownBox")
            .onclick = () => {

            document
                .getElementById("skipModal")
                .classList.remove("hidden");
        };

        document
            .getElementById("closeSkipBtn")
            .onclick = () => {
